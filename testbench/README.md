...
* **Robuste COM-Port-Kommunikation auf Windows-Runnern:**
  ...
  - **Klassenweite Test-Fixture:** In `test_epd_lifecycle.py` wird die serielle Verbindung über eine Klassen-Fixture (`scope="class"`) für den gesamten Testzyklus (`test_00` bis `test_09`) dauerhaft aufrechterhalten, statt sie zwischen den Tests wiederholt zu schließen und neu zu öffnen.
...
| :--- | :--- | :--- |
| **0** | `test_00_factory_reset_via_power_cycles` | Deaktiviert das Gerät vorab über die REST-API (`POST /activatedevice reset: True`), setzt es per mindestens 6 aufeinanderfolgende Relais-Power-Cycles auf Werkseinstellungen zurück (`StartCounter >= 5`) und verifiziert `[MAIN] Reset - ACT 1 \| WIFI 1`. |
...
| **2** | `test_02_device_activation` | Ruft REST-API `POST /activatedevice` auf und wartet auf den autonomen Handshake des Displays (`[AWS RX] Device is activated`), sodass das Gerät für AWS IoT und OTA autorisiert ist. |
| **3** | `test_03_production_firmware_ota` | Flasht die offizielle Produktions-Firmware via OTA-Manifest JSON (`http://<bucket-domain>/espfota_<target>.json`) und verifiziert den Reboot in die Produktionsversion (z. B. V3.0.17). |
| **4** | `test_04_candidate_firmware_ota` | Lädt das frisch gebuildete Firmware-Binary temporär als `test-firmware-<target>.bin` nach S3 hoch, triggert Direkt-OTA via URL, verifiziert den Reboot in die Kandidaten-Version und löscht die Test-Binärdatei anschließend aus S3. |
| **5** | `test_05_candidate_factory_reset` | Führt erneut einen 6x Power-Cycle Factory-Reset auf der neu geflashten Kandidaten-Firmware durch und deaktiviere das Gerät via REST-API zur Verifikation des Reset-Verhaltens auf der neuen Version. |
| **6** | `test_06_candidate_ble_wifi_provisioning` | Prüft die BLE-WLAN-Provisionierung auf der werksfrisch zurückgesetzten Kandidaten-Firmware. |
| **7** | `test_07_candidate_device_activation` | Aktiviert die Kandidaten-Firmware über die REST-API und validiert den erfolgreichen autonomen AWS-Handshake. |
| **8** | `test_08_candidate_picture_render_and_payload` | Fordert Presigned URL an (`POST /uploads`), lädt Testbild via HTTP PUT hoch, weckt Display auf, prüft Download (`[DL] Done`), Rendering (`[EPD] Set Image Done`) und Quittung in DynamoDB `iotPayload`. |
| **9** | `test_09_candidate_deactivate_and_deep_sleep` | Deaktiviert das Gerät per REST-API (`reset: True`), bereinigt Testbilder und verifiziert den sauberen Eintritt in Deep Sleep (`[MAIN] Going to Sleep for 0 seconds`). |
